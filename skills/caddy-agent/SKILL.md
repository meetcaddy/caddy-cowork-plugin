...
3. **Portal login.** First ask: "Which portal — dev or production? (If you're not sure, it's production.)" — always, per `<portal-targets>` rule 4, unless the user already named one this conversation. Pass the answer explicitly (`portal: "dev"` or `portal: "prod"`). Call `graph_login`, **announce which environment the link targets** ("this login targets the DEV portal"), then surface the `authorize_url` prominently — one line, one link, their code is prefilled, they're choosing which orgs this device may read. **Then, in the SAME turn — do not stop, do not ask the user to report back — immediately call `graph_auth_status` with `wait_seconds: 120` AND the `device_code` + `portal` values from `graph_login`'s result. Passing those two fields is mandatory on every status call** — they carry the login through even when each tool call runs in a fresh process or sandbox. The call blocks server-side while the user approves in the browser; the instant the portal confirms, it returns authenticated (credentials saved) and you continue straight into the graph pull. If it returns `waiting_for_authorization`, call it again with the same arguments and keep riding — the user should click Approve in their browser and watch the flow complete without typing anything. Only on `expired` or `denied` do you come back to the user: say what happened and offer a fresh code.
